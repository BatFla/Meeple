...
try {
    ...
    // Accepter la demande
    $update_stmt = $conn->prepare("UPDATE linkspw179.friends SET status = 'accepted' WHERE user_id = ? AND friend_id = ?");
    $update_stmt->execute([$friend_id, $user_id]);

    header('Location: profile.php?success=request_accepted');
    exit;
